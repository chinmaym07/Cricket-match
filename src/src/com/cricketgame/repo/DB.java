...
public class DB {
    ...
    public static void createTeam(Team team, String teamName) throws ClassNotFoundException, SQLException {
        try {
            ...
            ArrayList<Player> players = team.getPlayersArr();
            while (rs1.next()) {
                ...
                if (playerName.equals(captainName))
                    team.setCaptainName(playerName);
    ...
    public static void updateTossDetails(Toss tossObj, int matchId) throws SQLException {
        try {
            ...
            String sql = "insert into toss (matchId, teamIdWhoWonTheToss, teamIdWhoTookTheCall, teamIdWhoWillBat, teamIdWhoWillBowl, tossOutcome, callersChoice) values (" +
                    matchId + "," + tossObj.getTossWinner() + "," + tossObj.getWhoTookTheCall() + "," + tossObj.getWhoWillBat() + "," + tossObj.getWhoWillBowl() + ",'"
                    + tossObj.getOnTossOutcome() + "','" + tossObj.getCallerChoice() + "')";
    ...
    public static void storeMatchData(Match match) throws SQLException {
        try {
            ...
            String sql = "insert into matches (matchId, teamAId, teamBId, teamIdWhoWontheToss, matchOver) values (" +
                    match.getMatchId() + "," + match.getTeamA().getTeamId() + "," + match.getTeamB().getTeamId() + "," + teamIdWhoWonTheToss + "," + match.getMatchOvers() + ")";
            int count = st.executeUpdate(sql);
        } catch (SQLException e) {
            e.printStackTrace();
            System.out.println(e);
    ...
    public static void updateMatchData(Match match) throws SQLException {
        try {
            Statement st = connection.createStatement();
            int teamIdWhoWonMatch = match.getMatchWinner().equals(MatchWinnerEnums.MatchWinner.TEAMA) ? match.getTeamA().getTeamId() : match.getTeamB().getTeamId();
    ...
    public static void updatePlayerStats(ArrayList<Player> players,int matchId) throws SQLException {
        try {
            ...
            for (Player currentPlayer : players) {
                ...
                Statement st = connection.createStatement();
                String sql = "Insert into playerstats (matchId, playerId, runsScored, runsGiven, wicketsTaken, ballsFaced, noOfNoBalls, noOfWideBalls,maidenOvers,ballsBowled,oversBowled) values (" +
                        matchId+","+currentPlayer.getPlayerId() + "," + currentPlayerStats.getRunsScored() + "," + currentPlayerStats.getRunsGiven() + "," + currentPlayerStats.getWicketsTaken() + "," + currentPlayerStats.getBallsFaced() +
                        "," + currentPlayerStats.getNoOfNoBalls() + "," + currentPlayerStats.getNoOfWideBalls() + "," + currentPlayerStats.getMaidenOvers() + "," + currentPlayerStats.getBallsBowled() + "," + currentPlayerStats.getOversBowled() + ")";
    ...
    public static void updateInningsStats(int matchId, Innings innings) throws SQLException {
        try {
            ...
            Statement st = connection.createStatement();
            String sql = "Insert into innings (inningsId, matchId, battingTeamId, bowlingTeamId, totalScore, wicketsFallen, extraRuns, noOfNoBalls, noOfWideBalls, oversBatted) values (" +
                    innings.getInningsId() + "," + matchId + "," + innings.getBattingTeamId() + "," + innings.getBowlingTeamId() + "," + innings.getTotalScore() + ","
                    + innings.getFallOfWickets() + "," + innings.getExtraRuns() + "," + innings.getNoOfNoBalls() + "," + innings.getNoOfWideBalls() + "," + innings.getOversBatted() + ")";
            count = st.executeUpdate(sql);
        } catch (SQLException e) {
            e.printStackTrace();
            System.out.println(e);
        ...
        try {
            ...
            for (WicketsHistory currentWicket : wicketsHistories) {
                ...
                String sql = "Insert into wicketshistory (inningsId,matchId, batsmanId, bowlerId, wicketsFallenInOver, runScored, wicketsDown) values (" +
                        inningsId + "," + matchId + "," + currentWicket.getBatsmanId() + "," + currentWicket.getBowlerId() + "," + currentWicket.getWickerFallenInOver() + "," + currentWicket.getRunScored() + "," + currentWicket.getWicketsDown() + ")";
                count += st.executeUpdate(sql);
            }
        } catch (SQLException e) {
            e.printStackTrace();
